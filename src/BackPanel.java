...
public class BackPanel extends JPanel {
    ...
    public BackPanel(Room room) {//AnimalPanel 클래스는 room3 객체를 전달받아 room3와 상호작요 할 수 있다. 이걸로 인해 room3의 버튼에 이미지를 씌울수 있다.
        this.room=  room;
        ...
        for (int i = 0; i < 6; i++) {
            String imagePath = "img/back" + (i + 1) + ".png";//버튼마다 각각 다른 이미지를 넣어준다
        ...
        buttons[6].addActionListener(e->{if (room.getSizecheck()==1){//확대할때
            ...
            int newWidth = 400; // 변경할 이미지의 너비
            int newHeight = 240; // 변경할 이미지의 높이

            ImageIcon resizedIcon = resizeImageIcon(save, newWidth, newHeight);
            room.setGamingBack(resizedIcon);


        }
        else if(room.getSizecheck()==0){//축소할 때

            int newWidth = 260; // 변경할 이미지의 너비
            int newHeight = 190; // 변경할 이미지의 높이

            ImageIcon resizedIcon = resizeImageIcon(save, newWidth, newHeight);
            room.setGamingBack(resizedIcon);; // room3 클래스의 AnimalB 버튼에 아이콘(이미지임)을 설정하는 메서드 호출
        ...
        explain= new JLabel("각 상품은 10000씩 비싸집니다."); // 추가된 부분
    ...
    private class ButtonClickListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (room.getSizecheck()==1 && ControlMoney.money>=((ivalue+1)*10000)){//room3가 확대되어 있는 상태일 때.
                ...
                int newHeight = 190; // 변경할 이미지의 높이
                ImageIcon resizedIcon = resizeImageIcon(icon, newWidth, newHeight);
                room.setGamingBack(resizedIcon); // room 클래스의 BedB 버튼에 아이콘(이미지임)을 설정하는 메서드 호출
            }
