...
public class AnimalPanel extends JPanel {
    ...
    public AnimalPanel(Room room) {//AnimalPanel 클래스는 room3 객체를 전달받아 room3와 상호작요 할 수 있다. 이걸로 인해 room3의 버튼에 이미지를 씌울수 있다.
        this.room=  room;
        ...
        for (int i = 0; i < 6; i++) {
            String imagePath = "img/pet" + (i + 1) + ".png";//버튼마다 각각 다른 이미지를 넣어준다
            icons[i] = createResized(imagePath, 100, 100);//가로 세로 100으로 넣어준다.
            buttons[i] = new JButton(icons[i]);//버튼에 이미지를 넣어준다
            buttons[i].setBackground(Color.WHITE);//백그라운드는 하얀색으로
            buttons[i].addActionListener(new ButtonClickListener(icons[i]));//버튼에 actionListener를 추가해준다.
            add(buttons[i]);
        }
        ...
        buttons[6].setBackground(Color.WHITE);
            ...
            buttons[6].addActionListener(e->{if (room.getSizecheck()==1){//확대할때
                ...
                int newWidth = 130; // 변경할 이미지의 너비
                int newHeight = 130; // 변경할 이미지의 높이

                ImageIcon resizedIcon = resizeImageIcon(save, newWidth, newHeight);
                room.setAnimalBIcon(resizedIcon);


            }
            else if(room.getSizecheck()==0){//축소할 때

                int newWidth = 100; // 변경할 이미지의 너비
                int newHeight = 100; // 변경할 이미지의 높이

                ImageIcon resizedIcon = resizeImageIcon(save, newWidth, newHeight);
                room.setAnimalBIcon(resizedIcon); // room3 클래스의 AnimalB 버튼에 아이콘(이미지임)을 설정하는 메서드 호출
    ...
    private class ButtonClickListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (room.getSizecheck()==1 && ControlMoney.money>=1000){//room3가 확대되어 있는 상태일 때.
                ...
                ImageIcon resizedIcon = resizeImageIcon(icon, newWidth, newHeight);
                room.setAnimalBIcon(resizedIcon); // room 클래스의 AnimalB 버튼에 아이콘(이미지임)을 설정하는 메서드 호출
