...
public class ToiletPanel extends JPanel {
    ...
    public ToiletPanel(Room room) {//AnimalPanel 클래스는 room3 객체를 전달받아 room3와 상호작요 할 수 있다. 이걸로 인해 room3의 버튼에 이미지를 씌울수 있다.
        this.room=  room;
        ...
        for (int i = 0; i < 6; i++) {
            String imagePath = "img/변기" + (i + 1) + ".png";//버튼마다 각각 다른 이미지를 넣어준다
        ...
        buttons[6].addActionListener(e->{if (room.getSizecheck()==1){//확대할때
            ...
            room.setTIcon(resizedIcon);


        }
        else if(room.getSizecheck()==0){//축소할 때

            int newWidth = 150; // 변경할 이미지의 너비
            int newHeight = 150; // 변경할 이미지의 높이

            ImageIcon resizedIcon = resizeImageIcon(save, newWidth, newHeight);
            room.setTIcon(resizedIcon); // room3 클래스의 AnimalB 버튼에 아이콘(이미지임)을 설정하는 메서드 호출
        ...
        explain= new JLabel("각 변기는 5000씩 비싸집니다."); // 추가된 부분
    ...
    private class ButtonClickListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (room.getSizecheck()==1 && ControlMoney.money>=((ivalue+1)*5000)){//room3가 확대되어 있는 상태일 때.
                ...
                save=icon;
                int newWidth = 200; // 변경할 이미지의 너비
                int newHeight = 200; // 변경할 이미지의 높이
                ImageIcon resizedIcon = resizeImageIcon(icon, newWidth, newHeight);
                room.setTIcon(resizedIcon);
            }
            else if(room.getSizecheck()==0 && ControlMoney.money>=((ivalue+1)*5000)){//room이 축소 되어있는 상태일 때
                ControlMoney.money-=((ivalue+1)*5000);
                ControlMoney.MoneyLabel.setText("Money "+ControlMoney.money);
                save=icon;
                int newWidth = 150; // 변경할 이미지의 너비
                int newHeight = 150; // 변경할 이미지의 높이
                ImageIcon resizedIcon = resizeImageIcon(icon, newWidth, newHeight);
                room.setTIcon(resizedIcon); // room 클래스의 BedB 버튼에 아이콘(이미지임)을 설정하는 메서드 호출
            }
